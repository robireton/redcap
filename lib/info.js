...
// REDCap serializes project flags as 1/0 in most versions but as "1"/"0" in some,
// and Boolean('0') is true — so compare rather than coerce.
const flag = v => v === true || v === 1 || v === '1'

// empty-string fields mean “not set”; a blank datetime is not a parse failure
const datetime = v => v ? new Date(REDCapDatetime.parse(v)) : null
...
export default class REDCapProjectInformation {
  ...
  get creationTime () {
    return datetime(this.#creation_time)
  }

  get productionTime () {
    return datetime(this.#production_time)
  }

  get inProduction () {
    return flag(this.#in_production)
  }
  ...
  get isLongitudinal () {
    return flag(this.#is_longitudinal)
  }

  get hasRepeatingInstrumentsOrEvents () {
    return flag(this.#has_repeating_instruments_or_events)
  }

  get surveysEnabled () {
    return flag(this.#surveys_enabled)
  }

  get schedulingEnabled () {
    return flag(this.#scheduling_enabled)
  }

  get recordAutonumberingEnabled () {
    return flag(this.#record_autonumbering_enabled)
  }

  get randomizationEnabled () {
    return flag(this.#randomization_enabled)
  }

  get ddpEnabled () {
    return flag(this.#ddp_enabled)
  }
  ...
  get displayTodayNowButton () {
    return flag(this.#display_today_now_button)
  }
  ...
  get bypassBranchingEraseFieldPrompt () {
    return flag(this.#bypass_branching_erase_field_prompt)
  }
}
